...
给定 t组询问，每组询问给定三个整数 n , m , p  ，其中 p 是质数，请你输出  
$$
C_m^n(\bmod\ p)
$$
 1≤t≤20, 1≤m≤n≤10^18,1≤p≤10e5

组合数公式，如果p>m,可以用下面这个公式，求m!的逆元和(n-m)!的逆元
...
$$
可当 m ⩾ p 的时候，分母的乘法逆元可能不存在。（因为 x 是 p 的倍数的话 x 就没有模 p 的逆元）这里就要用lucas定理来求了
$$
